...
//go:generate mockgen -package mocks -destination mocks/mocks.go github.com/ibiscum/Hands-On-Software-Engineering-with-Golang/Chapter10/linksrus/service/frontend GraphAPI,IndexAPI
//go:generate mockgen -package mocks -destination mocks/mock_indexer.go github.com/ibiscum/Hands-On-Software-Engineering-with-Golang/Chapter06/textindexer/index Iterator
...
func (svc *Service) runQuery(searchTerms string, offset uint64) ([]matchedDoc, *paginationDetails, error) {
	...
	if offset > uint64(^uint(0)) {
		nextPageOffset = int(^int(0)) // max int value
	} else if offset+uint64(len(matchedDocs)) > uint64(^uint(0)) {
		nextPageOffset = int(^int(0)) // max int value
	} else {
